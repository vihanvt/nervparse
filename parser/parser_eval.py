...
def evaluate(model, sentences, gold_dependencies, wordvocab, posvocab, labelvocab):
    ...
    correct_unlabeled = 0
    total = 0
    predicted_dependencies = minibatch_parse(sentences, model, batch_size=32,wordvocab=wordvocab, posvocab=posvocab, labelvocab=labelvocab)
    for sent_idx, (predicted, gold) in enumerate(zip(predicted_dependencies, gold_dependencies)):
        pred_deps = {}
        for head, dep in predicted:
            pred_deps[dep["index"]] = head["index"]
        for dep_info in gold:
            depid, head_idx = dep_info[0], dep_info[1]
            total += 1
            if depid in pred_deps and pred_deps[depid] == head_idx:
                correct_unlabeled += 1
    #uas calculation- las to be done
    uas = (correct_unlabeled / total) * 100 if total > 0 else 0
    
    print(f"Total dependencies: {total}, Correct unlabeled: {correct_unlabeled}")
    return uas
...
uas_dev = evaluate(model, dev_sentences, gold_dependencies_dev, wordvocab, posvocab, labelvocab)
uas_test = evaluate(model, test_sentences, gold_dependencies_test, wordvocab, posvocab, labelvocab)

print(f"Dev UAS: {uas_dev:.2f}%")
print(f"Test UAS: {uas_test:.2f}%")
